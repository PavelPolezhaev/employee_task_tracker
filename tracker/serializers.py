...
class ImportantTaskSerializer(ModelSerializer):
    ...
    def get_available_employees(self, task):
        ...
        least_loaded = [
            ...
            if emp['task_count'] == min_tasks
        ]

        parent_candidates = [
            employees_dict[emp_id] for emp_id in parent_employee_ids
            if emp_id in employees_dict and
               employees_dict[emp_id]['task_count'] <= min_tasks + 2
        ]
        ...
        return [
            ...
            for emp in all_candidates.values()
        ]
